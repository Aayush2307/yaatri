    ...
    <main className="min-h-screen bg-bg-surface pb-24">
      <section className="relative overflow-hidden bg-indigo-deepest px-5 pb-5 pt-6 text-star-white">
        ...
        <div className="relative z-10 flex items-center justify-between">
          ...
          <Link href="/account" className="h-8 w-8 rounded-full bg-amethyst/25 text-center text-[12px] leading-8">{name[0] || 'Y'}</Link>
        </div>
        <p className="pt-5 text-[10px] uppercase tracking-[0.1em] text-gold-warm">Good morning, {name}</p>
        <h1 className="pt-2 font-serif text-[30px] font-light leading-[1.18]">From <span className="italic text-amethyst">sankalp</span> to darshan — every step held.</h1>
        <div className="mt-4 rounded-card border-[0.5px] border-[rgba(200,184,255,0.2)] bg-[rgba(127,119,221,0.12)] p-3">
          <p className="text-[13px]">{panchangCard.tithi} · {panchangCard.sanskrit}</p>
          <p className="pt-1 text-[11px] text-star-white/70">{panchangCard.muhurat}</p>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2">
          <Link href="/plan" className="flex min-h-[44px] items-center justify-center rounded-[11px] bg-indigo-mid text-[13px]">Plan my yatra</Link>
          <Link href="/concierge" className="flex min-h-[44px] items-center justify-center rounded-[11px] border-[0.5px] border-amethyst text-[13px]">Concierge</Link>
        </div>
      </section>
      ...
      <section className="space-y-3 px-5 pt-4">
        ...
        <div>
          ...
          <div className="no-scrollbar flex gap-3 overflow-x-auto pb-2">
            {circuits.map((c) => (
              <Link key={c.id} href="/explore" className="min-w-[148px] rounded-card border-[0.5px] border-divider bg-white p-2">
                <div className="h-[82px] rounded-[10px]" style={{ background: c.gradient }} />
                <p className="pt-2 text-[13px] text-text-dark">{c.name}</p>
                <p className="text-[11px] text-text-muted">{c.stops}</p>
                <p className="pt-1 text-[11px] text-text-mid">{c.price} · {c.nights}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-card border-[0.5px] border-divider bg-white p-4">
          ...
          <Link href="/concierge" className="mt-2 inline-flex min-h-[44px] items-center rounded-[10px] bg-indigo-mid px-4 text-[13px] text-star-white">Open concierge</Link>
        </div>
      </section>
