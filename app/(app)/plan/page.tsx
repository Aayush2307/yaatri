  ...
  return (
    <main className="min-h-screen bg-bg-surface px-5 pb-24 pt-6">
      <div className="mx-auto max-w-md space-y-3">
        ...
        {circuits.map((c) => (
          <button key={c.id} onClick={() => setSelected(c.id)} className="w-full rounded-card border-[0.5px] p-3 text-left" style={{ borderColor: selected === c.id ? '#534AB7' : 'rgba(127,119,221,0.3)', backgroundColor: selected === c.id ? 'rgba(83,74,183,0.08)' : 'white' }}>
            <p className="text-[13px] text-text-dark">{c.name}</p>
            <p className="text-[11px] text-text-muted">{c.stops}</p>
          </button>
        ))}
        <Link href="/concierge" className="flex min-h-[44px] items-center justify-center rounded-[11px] bg-indigo-mid text-[13px] text-star-white">Send to Meera →</Link>
      </div>
